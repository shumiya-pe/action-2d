...
ゲーム固有ロジックと基盤を分離しており、同じ基盤の上で複数タイトルを開発できる。

収録ゲーム（`src/games/`）:

| ゲーム       | 内容                                                             | 状態                 |
| ------------ | ---------------------------------------------------------------- | -------------------- |
| `proto`      | 横スクロールアクション試作（ジャンプ・斬撃・射撃＋強化アイテム） | **起動対象（既定）** |
| `platformer` | マリオ風の最小構成サンプル（縦切り1本目）                        | 参照用に保持         |

起動対象の切り替えは `src/ui/GameCanvas.tsx` の import を差し替える。
...
└── games/
    ├── proto/            # 横スクロールアクション試作（起動対象）
    │   ├── index.ts      # GameDefinition（シーン一覧・解像度・重力）
    │   ├── constants.ts  # 調整パラメータ・キー定義
    │   ├── skin.ts       # 仮デザイン（テクスチャ生成）の集約点
    │   ├── level.ts      # ステージデータ
    │   ├── scenes/       # Boot / Preload / Title / Game / GameOver
    │   ├── objects/      # Player / Enemy
    │   └── systems/      # StatusSystem（バフ・武装の時限管理）
    └── platformer/       # マリオ風の最小構成サンプル（参照用）
```
...
- **手動スクロール**: 左右移動あり（通常のアクション）
- **自動スクロール**: プレイヤーが自動で右へ走り続ける（オートラン）
...
ルール: コイン +100 点 / walker（小型敵）撃破 +200 点 / brute（大型敵、HP3）撃破 +500 点。
敵は踏む・斬る・撃つで倒せる。横から触れるとミス。穴に落ちてもミス。右端の旗でクリア。

アイテム（取ると一定時間だけ有効）:

- ● 水色 = **飛行**（空中でジャンプ長押しで上昇、10秒）
- ● 緑 = **速度アップ**（移動 1.5 倍、8秒）
- ● 赤 = **攻撃力アップ**（与ダメージ2倍、8秒）
- ■ 橙 = **爆弾**（射撃が放物線の爆弾になり範囲爆発、12秒）
- ■ 紫 = **弓**（射撃が高速・貫通の矢になる、12秒）
